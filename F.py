...
class ContentBasedNewsRecommender:
    def __init__(self, data_path=None, stopwords_path=None):
        self.data = None
...
def main():
        ...
        if st.button("获取推荐"):
            ...
            try:
                ...
                if 'recommender' not in st.session_state:
                    st.session_state.recommender = ContentBasedNewsRecommender("data1_output.csv", "stopwords.txt")
    ...
    with col2:
        st.header("使用提示")
        st.markdown("- 目前共摘录新浪新闻财经、国内、国际、体彩、娱乐五个频道约六千条新闻")
